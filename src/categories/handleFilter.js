...
exports.filterTopicsByGroup =
    exports.populateDropdown =
    exports.fetchGroups =
        void 0;
    ...
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const response = yield fetch(
                `/api/topic?group=${encodeURIComponent(groupName)}`,
            );
            if (!response.ok) {
                throw new Error(
                    `Error fetching topics for group ${groupName}: ${response.statusText}`,
                );
            }
            ...
            console.error(
                `An error occurred while fetching topics for group ${groupName}: ${error}`,
            );
