...
module.exports = function (Posts) {
    Posts.getUserInfoForPosts = async function (uids, uid) {
        ...
        const result = await Promise.all(
            userData.map(async (userData) => {
                ...
                if (
                    userData.isPrivate &&
                    !userHasPrivilegeToViewPrivatePost(uid, userData)
                ) {
    ...
    async function userHasPrivilegeToViewPrivatePost(uid, userData) {
        ...
        assert.strictEqual(
            typeof (await privsUsers.isAdministrator(uid)),
            "boolean",
            "isAdministrator should return a boolean",
        );
        ...
        if (userData.isPrivate) {
            assert.strictEqual(
                typeof userData.isPrivate,
                "boolean",
                "userData.isPrivate should be a boolean",
            );
            assert.strictEqual(
                isAdmin,
                true,
                "User should be admin to view private post",
            );
        }
