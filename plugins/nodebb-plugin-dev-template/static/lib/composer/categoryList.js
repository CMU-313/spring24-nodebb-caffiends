...
define("composer/categoryList", [
        ...
        postContainer
            ...
            .translateText(
                selector.selectedCategory
                    ...
                    : "[[modules:composer.select_category]]",
            )
            .on("click", function () {
    ...
    categoryList.updateTaskbar = function (postContainer, postData) {
        if (parseInt(postData.cid, 10)) {
            api.get(`/categories/${postData.cid}`, {}).then(
                function (category) {
    ...
    async function changeCategory(postContainer, postData, selectedCategory) {
        ...
        const categoryData = await window
            .fetch(
                `${config.relative_path}/api/category/${selectedCategory.cid}`,
            )
            .then((r) => r.json());
        postData.category = categoryData;
        updateTaskbarByCategory(postContainer, categoryData);
        require(["composer/scheduler", "composer/tags"], function (
            scheduler,
            tags,
        ) {
